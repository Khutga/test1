...
class SqlServis {
  ...
  // =========================================================================
  // BAŞARIM VE GÜNLÜK GİRİŞ KONTROLLERİ
  // =========================================================================

  static Future<void> gunlukGirisKontrol(int kullaniciId) async {
    var res = await cek(tablo: Tablolar.hesaplar, sartlar: {'id': kullaniciId});
    if (!res.basarili || res.veri.isEmpty) return;

    var user = res.veri.first;
    String sonGirisStr = user['son_giris_tarihi']?.toString() ?? '';
    int seri = int.tryParse(user['giris_serisi']?.toString() ?? '0') ?? 0;
    
    DateTime simdi = DateTime.now();
    DateTime bugun = DateTime(simdi.year, simdi.month, simdi.day);
    
    DateTime sonGirisTarihi;
    if (sonGirisStr.isNotEmpty) {
      sonGirisTarihi = DateTime.tryParse(sonGirisStr) ?? bugun.subtract(const Duration(days: 2));
    } else {
      sonGirisTarihi = bugun.subtract(const Duration(days: 2));
    }
    
    DateTime sonGirisGun = DateTime(sonGirisTarihi.year, sonGirisTarihi.month, sonGirisTarihi.day);

    int farkGun = bugun.difference(sonGirisGun).inDays;

    if (farkGun == 0) {
      // Bugün zaten girmiş, hiçbir şey yapma (seriyi artırma).
      return; 
    } else if (farkGun == 1) {
      // Dün de girmiş, seriyi 1 artır.
      seri += 1;
    } else {
      // 1 günden fazla ara vermiş, seriyi başa sar.
      seri = 1;
    }
    ...
    await guncelle(
      tablo: Tablolar.hesaplar,
      veriler: {
        'giris_serisi': seri.toString(),
        'son_giris_tarihi': simdi.toIso8601String()
      },
      sartlar: {'id': kullaniciId.toString()}
    );
  }
  static Map<String, int>? _cachedAuraLimits;

  static Future<Map<String, int>> getAuraLimits() async {
    if (_cachedAuraLimits != null) return _cachedAuraLimits!;

    var res = await cek(tablo: 'sistem_ayarlari');
    Map<String, int> limitler = {
      'silver': 4,
      'gold': 8,
      'neon': 15,
      'galaxy': 30,
    ...
    if (res.basarili && res.veri.isNotEmpty) {
      for (var item in res.veri) {
        if (item['ayar_adi'] == 'aura_silver') limitler['silver'] = int.tryParse(item['ayar_degeri'].toString()) ?? limitler['silver']!;
        if (item['ayar_adi'] == 'aura_gold') limitler['gold'] = int.tryParse(item['ayar_degeri'].toString()) ?? limitler['gold']!;
        if (item['ayar_adi'] == 'aura_neon') limitler['neon'] = int.tryParse(item['ayar_degeri'].toString()) ?? limitler['neon']!;
        if (item['ayar_adi'] == 'aura_galaxy') limitler['galaxy'] = int.tryParse(item['ayar_degeri'].toString()) ?? limitler['galaxy']!;
      }
      _cachedAuraLimits = limitler;
    }
    
    return limitler;
  }

  static void auraOnbellekTemizle() {
    _cachedAuraLimits = null;
  }
}
