...
class _AjanslarTabState extends State<_AjanslarTab> {
  ...
  Future<void> _yukle() async {
    ...
    if (res.basarili) _ajanslar = res.veri;
    setState(() => _isLoading = false);
  }

  // Sahip ID'sinden isim çeken yardımcı
            ...
            child: GlassContainer(
              onTap: () {
                Navigator.push(context, MaterialPageRoute(
                  builder: (_) => _AjansDetaySayfasi(ajans: a),
                ));
              },
              padding: const EdgeInsets.all(14),
              child: Row(
...
// ─── AJANS DETAY SAYFASI (Üye Listesi) ───
class _AjansDetaySayfasi extends StatefulWidget {
  final Map<String, dynamic> ajans;
  const _AjansDetaySayfasi({required this.ajans});
  @override
  State<_AjansDetaySayfasi> createState() => _AjansDetaySayfasiState();
}

class _AjansDetaySayfasiState extends State<_AjansDetaySayfasi> {
  ...
  Future<void> _yukle() async {
    ...
    if (res.basarili) _uyeler = res.veri;
    setState(() => _isLoading = false);
  }
                  ...
                  return Padding(
                    ...
                    child: GestureDetector(
                      ...
                      child: Container(
                        padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 8),
                        decoration: BoxDecoration(
                          color: selected ? renk : Colors.transparent,
                          borderRadius: BorderRadius.circular(12),
                          border: Border.all(color: selected ? renk : context.border),
                        ),
...
class _IliskilerTab extends StatefulWidget {
  const _IliskilerTab();
  @override
  State<_IliskilerTab> createState() => _IliskilerTabState();
}

class _IliskilerTabState extends State<_IliskilerTab> {
  List<Map<String, dynamic>> _iliskiler = [];
  bool _isLoading = true;

  // İsim cache'i — her ID için sadece 1 kere DB'ye gider
      ...
      builder: (ctx) => Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              children: [
                Expanded(child: _adminField("İlişki Puanı", puanC, isNumber: true)),
                const SizedBox(width: 10),
                Expanded(child: _adminField("Seviye", seviyeC, isNumber: true)),
              ],
            ),
            const SizedBox(height: 16),
            Row(
                  ...
                  child: PremiumButton(
                    ...
                    onPressed: () async {
                      await SqlServis.guncelle(
                        ...
                        veriler: {
                          'iliski_puani': int.tryParse(puanC.text) ?? 0,
                          'seviye': int.tryParse(seviyeC.text) ?? 1,
                        },
                        sartlar: {'id': rel['id']},
                      );
                      Navigator.pop(ctx);
        ...
        itemBuilder: (_, i) {
          ...
          return Container(
            ...
            child: GlassContainer(
              ...
              child: Row(
                children: [
                  // İki kullanıcı avatarı
                  FutureBuilder<String>(
                    future: _isimCek(id1),
                    builder: (_, s1) => GlowAvatar(initial: (s1.data ?? '?')[0].toUpperCase(), radius: 18, color: AppTheme.accent),
                  ),
                  const Padding(
                    padding: EdgeInsets.symmetric(horizontal: 4),
                    child: Icon(LucideIcons.heart, color: AppTheme.danger, size: 14),
                  ),
                  FutureBuilder<String>(
                    future: _isimCek(id2),
                    builder: (_, s2) => GlowAvatar(initial: (s2.data ?? '?')[0].toUpperCase(), radius: 18, color: AppTheme.danger),
                  ),
                  const SizedBox(width: 12),
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        FutureBuilder<List<String>>(
                          ...
                          builder: (_, snap) => Text(
                            snap.hasData ? "${snap.data![0]} ❤️ ${snap.data![1]}" : "Yükleniyor...",
                  ...
                  Column(
                    crossAxisAlignment: CrossAxisAlignment.end,
                    children: [
                      _adminBadge("Lv.$seviye", AppTheme.accent),
                      const SizedBox(height: 2),
