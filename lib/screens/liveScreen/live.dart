...
import '../../services/sql_servis.dart'; // SQL işlemleri için
...
class _PremiumLiveStreamPageState extends State<PremiumLiveStreamPage> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    _connectToLiveKit();
    _chatMessages.add(ChatMessage(sender: "Sistem", text: "${widget.roomName} yayınına bağlanılıyor...", isSystem: true));
  }
    ...
    // Odadaki kişi sayısı değiştiğinde izleyici sayısını güncelle
      ...
      _listener?.on<TrackSubscribedEvent>((event) {
        ...
        if (event.track is VideoTrack && event.participant != null) {
          setState(() => _remoteVideoTracks[event.participant.identity] = event.track as VideoTrack);
        }
      });
      ...
      _listener?.on<DataReceivedEvent>((event) {
        ...
        if (type == 'chat') {
          ...
          _addChatMessage("Sistem", "${msgData['sender']}, $receiver'a $icon gönderdi!", isSystem: true);
          
          // PK Puanlarını Güncelle
          if (_isPkActive) {
            setState(() {
              if (receiver == widget.hostName) _hostScore += (points as int);
              else if (receiver == _pkOpponentName) _opponentScore += (points as int);
            });
          }
        } 
      ...
      if (widget.isHost) {
        ...
        if (publications != null && publications.isNotEmpty) {
          if (mounted) setState(() => _localVideoTrack = publications.first.track as VideoTrack?);
        }
      }
  ...
  void _startPk(String opponentName) async {
    ...
    _pkTimer = Timer.periodic(const Duration(seconds: 1), (timer) {
      if (_pkTimeLeft > 0) {
        ...
        _room?.localParticipant?.publishData(utf8.encode(jsonEncode({'type': 'pk_tick', 'time': _pkTimeLeft})));
      } else {
        _endPk();
      }
    });
  ...
  void _endPk() async {
    ...
    _room?.localParticipant?.publishData(utf8.encode(jsonEncode({'type': 'pk_end'})));

    // SQL'den sil
    if (widget.isHost) {
      await SqlServis.sil(tablo: 'aktif_pk', sartlar: {'oda_adi': widget.roomName});
    }
  }
  ...
  Future<bool> _onWillPop() async {
    if (widget.isHost) {
      ...
      if (_isPkActive) await SqlServis.sil(tablo: 'aktif_pk', sartlar: {'oda_adi': widget.roomName});
    }
  ...
  void _addChatMessage(String sender, String text, {bool isSystem = false}) {
    if (!mounted) return;
    setState(() {
      _chatMessages.insert(0, ChatMessage(sender: sender, text: text, isSystem: isSystem));
  ...
  void _sendMessage() async {
    if (_chatController.text.isNotEmpty && _room?.localParticipant != null) {
      ...
      final payload = jsonEncode({'type': 'chat', 'text': text, 'sender': widget.username});
  ...
  void _handleSendGift(String giftType, String emoji, int points) async {
    ...
    if (_room?.localParticipant != null) {
      ...
      try {
        await _room?.localParticipant?.publishData(utf8.encode(payload), reliable: true);
        _addChatMessage("Sistem", "Sen $_selectedGiftTarget'a $emoji gönderdin!", isSystem: true);
        
        if (_isPkActive) {
          setState(() {
            if (_selectedGiftTarget == widget.hostName) _hostScore += points;
            else if (_selectedGiftTarget == _pkOpponentName) _opponentScore += points;
          });
        }
      } catch (e) {
  ...
  void _triggerFloatingAnimation(String emoji) {
    ...
    setState(() => _floatingGifts.add(FloatingGift(id: giftId, left: 30 + _random.nextDouble() * 100, emoji: emoji)));
    Future.delayed(const Duration(seconds: 2), () {
      if (mounted) setState(() => _floatingGifts.removeWhere((g) => g.id == giftId));
    });
  }
  ...
  Widget _buildDynamicVideoLayout() {
    ...
    if (allTracks.isEmpty) return const Center(child: Text("Yayıncı bekleniyor...", style: TextStyle(color: Colors.white)));
    ...
    Widget buildTrackWithName(Map<String, dynamic> data) {
      return Stack(
        fit: StackFit.expand,
        children: [
          Container(color: Colors.black, child: VideoTrackRenderer(data['track'] as VideoTrack, fit: VideoViewFit.cover)),
          ...
          Positioned(
            ...
            child: Container(
              ...
              child: Text(data['name'], style: const TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.bold)),
            ),
          ),
        ],
      );
    ...
    return GridView.builder(
      ...
      gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 2, childAspectRatio: 0.7, crossAxisSpacing: 2, mainAxisSpacing: 2),
        ...
        body: MainBackground(
          child: Stack(
            ...
            children: [
                ...
                child: Row(
                  ...
                  children: [
                    Container(
                      ...
                      child: Row(
                        children: [
                          const CircleAvatar(radius: 14, backgroundColor: AppTheme.accent),
                          const SizedBox(width: 8),
                          Text(widget.hostName, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: Colors.white)),
                        ],
                      ),
                    ),
                    Row(
                    ...
                    child: Column(
                      children: [
                        Row(
                          mainAxisAlignment: MainAxisAlignment.spaceBetween,
                          children: [
                            Text("${widget.hostName}: $_hostScore", style: const TextStyle(fontSize: 12, color: AppTheme.accentLight)),
                            Text("PK BATTLE (0${_pkTimeLeft ~/ 60}:${(_pkTimeLeft % 60).toString().padLeft(2, '0')})", 
                              style: const TextStyle(fontSize: 12, color: Colors.white, fontWeight: FontWeight.bold)),
                            Text("$_pkOpponentName: $_opponentScore", style: const TextStyle(fontSize: 12, color: AppTheme.danger)),
                          ],
                        ),
              ...
              if (_isPkActive)
                Positioned(
                  ...
                  child: Container(
                    ...
                    child: Column(
                      children: [
                        const Text("Hedef", style: TextStyle(color: Colors.white70, fontSize: 10)),
                        const SizedBox(height: 4),
                        ChoiceChip(
                          label: Text(widget.hostName, style: const TextStyle(fontSize: 10)),
                          selected: _selectedGiftTarget == widget.hostName,
                          onSelected: (val) => setState(() => _selectedGiftTarget = widget.hostName),
                        ),
                        ChoiceChip(
                          label: Text(_pkOpponentName, style: const TextStyle(fontSize: 10)),
                          selected: _selectedGiftTarget == _pkOpponentName,
                          onSelected: (val) => setState(() => _selectedGiftTarget = _pkOpponentName),
                        ),
                      ],
                    ),
                  ),
              ...
              // Uçuşan Hediyeler
              ..._floatingGifts.map((gift) => Positioned(
                bottom: 150, left: gift.left,
                child: TweenAnimationBuilder<double>(
                  ...
                  builder: (context, value, child) {
                    return Transform.translate(
                      ...
                      child: Opacity(opacity: 1.0 - (value * value), child: Text(gift.emoji, style: const TextStyle(fontSize: 40))),
                    );
                  },
                ),
              )).toList(),
              ...
              // Chat Alanı
              Positioned(
                bottom: 120, left: 16, width: 250, height: 200,
                child: ShaderMask(
                  shaderCallback: (bounds) => const LinearGradient(
                    begin: Alignment.topCenter, end: Alignment.bottomCenter,
                    colors: [Colors.transparent, Colors.black, Colors.black], stops: [0.0, 0.2, 1.0],
                  ).createShader(bounds),
                        ...
                        child: Align(
                          ...
                          child: GestureDetector(
                            onTap: () {
                              ...
                              if (widget.isHost && !msg.isSystem && msg.sender != widget.hostName && !_isPkActive) {
                                showDialog(
                                  context: context,
                                  builder: (c) => AlertDialog(
                                    title: const Text("PK Başlat"),
                                    content: Text("${msg.sender} ile PK Battle başlatılsın mı?"),
                                    actions: [
                            ...
                            child: Container(
                              ...
                              decoration: BoxDecoration(
                                color: msg.isSystem ? AppTheme.accent.withOpacity(0.2) : Colors.black45,
                                borderRadius: BorderRadius.circular(16),
                              ),
                              child: RichText(
                                text: TextSpan(
                                  children: [
                                    if (!msg.isSystem) TextSpan(text: "${msg.sender}: ", style: const TextStyle(color: AppTheme.accentLight, fontWeight: FontWeight.bold, fontSize: 12)),
                                    TextSpan(text: msg.text, style: TextStyle(color: msg.isSystem ? AppTheme.accent : Colors.white, fontSize: 12)),
                                  ],
                                ),
                              ),
                            ),
              ...
              Positioned(
                ...
                child: Container(
                  padding: EdgeInsets.only(bottom: MediaQuery.of(context).padding.bottom + 10, left: 16, right: 16, top: 24),
                  ...
                  child: Column(
                    ...
                    children: [
                      Row(
                        ...
                        children: [
                          _buildGiftBtn("🐲", "Dragon", "25K", AppTheme.danger, () => _handleSendGift('Dragon', '🐲', 25000)),
                          _buildGiftBtn("🛳️", "Yacht", "8.5K", AppTheme.accentLight, () => _handleSendGift('Yacht', '🛳️', 8500)),
                          _buildGiftBtn("❤️", "Kalp", "10", AppTheme.danger, () => _handleSendGift('Heart', '❤️', 10)),
                        ],
                      ),
                      ...
                      Row(
                        children: [
                          Expanded(
                            child: TextField(
                              ...
                              decoration: InputDecoration(
                                hintText: widget.isHost ? "İzleyicilerine yaz..." : "Sohbete Katılın...",
                          ...
                          GestureDetector(
                            onTap: _sendMessage,
                            child: Container(padding: const EdgeInsets.all(12), decoration: const BoxDecoration(color: AppTheme.accent, shape: BoxShape.circle), child: const Icon(LucideIcons.send, size: 16, color: Colors.white)),
                          ),
                        ],
                      ),
                    ],
  ...
  Widget _buildGiftBtn(String emoji, String title, String cost, Color color, VoidCallback onTap) {
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(color: color.withOpacity(0.1), border: Border.all(color: color.withOpacity(0.3)), borderRadius: BorderRadius.circular(12)),
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
